...
public class LivroResource {
	...
	private RepositorioLivros livroRepo = RepositorioLivros.getInstance();
	
	@GET
	@Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) //PERMITE QUE SEJA RETORNADO TANTO JSON COMO XML
	...
	public Livro getLivroIsbn(@PathParam("isbn")String isbn) {
	 ...
	 //IRA RECEBER REQUISICOES DO TIPO POST  
     @POST
     ...
     public void removeLivro(@PathParam("id")Long id) {
    	 ...
    	 } catch (LivroNaoEncontradoException erro) {
    		 throw new WebApplicationException(Status.NOT_FOUND);
    	 }
     }
}
